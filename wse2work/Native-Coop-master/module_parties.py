...
#parties before this point are hardwired. Their order should not be changed.
    ...
    ] + [ ("player_party_%04d"%(player_index),"Player Party",icon_player|pf_disabled, no_menu, pt_none,fac_player_faction,0,ai_bhvr_hold,0,(17, 52.5),[(trp_player_0000+player_index,1,0)]) for player_index in range(0, players_number) ] + [
...
##  ("black_khergit_spawn_point"  ,"black_khergit_sp",pf_disabled|pf_is_static|pf_dont_send_to_clients, no_menu, pt_none, fac_outlaws,0,ai_bhvr_hold,0,(47.1, -73.3),[(trp_looter,15,0)]),
 ...
 # add extra towns before this point 
  ...
  # Temp party for client-side local SP battles (must be at END to not shift other party IDs)
  ("coop_local_enemy","{!}coop_local_enemy",pf_disabled|pf_dont_send_to_clients, no_menu, pt_none, fac_neutral,0,ai_bhvr_hold,0,(1,1),[]),
  # Kill-tally parties for client-side local SP battles. Isolated from the
  # vanilla p_player_casualties/p_total_enemy_casualties (mt_lead_charge's
  # count_mission_casualties_from_agents clears + miscategorises those on a
  # coop client). player_cas = our killed troops; enemy_cas = enemies defeated.
  ("coop_local_player_cas","{!}coop_local_player_cas",pf_disabled|pf_dont_send_to_clients, no_menu, pt_none, fac_neutral,0,ai_bhvr_hold,0,(1,1),[]),
  ("coop_local_enemy_cas","{!}coop_local_enemy_cas",pf_disabled|pf_dont_send_to_clients, no_menu, pt_none, fac_neutral,0,ai_bhvr_hold,0,(1,1),[]),
  ]
